...
public:
    ...
    moodycamel::ConcurrentQueue<string*> linePool;
    moodycamel::ConcurrentQueue<string*> freeLinePool;
    moodycamel::ConcurrentQueue<Position*> freePositionPool;
    moodycamel::ConcurrentQueue<Position*> outputPositionPool;
    ...
    ~Positions() {
        ...
        while(freePositionPool.try_dequeue(pos)) {
            delete pos;
        }
        // Unmap mmap
    ...
    void outputFunction(string outputFileName) {
        ...
        *out_ << "ref\tpos\tstrand\tconvertedBaseQualities\tconvertedBaseCount\tunconvertedBaseQualities\tunconvertedBaseCount\n";
        ...
        Position* pos;
        while (working || outputPositionPool.size_approx() > 0) {
            if (outputPositionPool.try_dequeue(pos)) {
                outputBuffer.clear();
        ...
        for (index = 0; index < refPositions.size(); index++) {
            if (refPositions[index]->location < refCoveredPosition - loadingBlockSize) {
                if (refPositions[index]->empty() || refPositions[index]->strand == '?') {
                    ...
                    outputPositionPool.enqueue(refPositions[index]);
                }
            } else {
                break;
            }
    ...
    void getFreeStringPointer(string*& newLine) {
        if (!freeLinePool.try_dequeue(newLine)) {
            newLine = new string();
        }
    }

    void getFreePosition(Position*& newPosition) {
        while (outputPositionPool.size_approx() >= 10000) {
            this_thread::sleep_for(std::chrono::microseconds(1));
        }
        if (!freePositionPool.try_dequeue(newPosition)) {
    ...
    void returnLine(string* line) {
        line->clear();
        freeLinePool.enqueue(line);
    }

    void returnPosition(Position* pos) {
        pos->initialize();
        freePositionPool.enqueue(pos);
    }
