...
public:
    ...
    Positions(string inputRefFileName, int inputNThreads, bool inputAddedChrName, bool inputRemovedChrName) {
        ...
        const size_t INITIAL_POOL_SIZE = 10000;
    
        // 預分配 Position 物件到 freePositionPool
        for (size_t i = 0; i < INITIAL_POOL_SIZE; ++i) {
            freePositionPool.push(new Position());
        }
        
        // 預分配 string 物件到 freeLinePool
        for (size_t i = 0; i < INITIAL_POOL_SIZE; ++i) {
            freeLinePool.push(new string());
        }
    ...
    ~Positions() {
        ...
        // 釋放 freeLinePool 中的物件
        string* str;
        while(freeLinePool.popFront(str)) {
            delete str;
        }
        // Unmap mmap
